...
print("prices between 90 and 120:", prices[(prices >= 90) and (prices <= 120)])
# Task 7:
# Print how many values in prices are greater than 100.
print("number of prices > 100:", np.sum(prices > 100))
# Task 8:
# Print how many values in prices are less than 90.
print("number of prices < 90:", np.sum(prices < 90))
# Task 9:
# Make a NumPy array named changes with these numbers:
# -2, 5, 0, -1, 3, 7, -4
changes = np.array([-2, 5, 0, -1, 3, 7, -4])
# Task 10:
# Print only the positive values from changes.
print("positive changes:", changes[changes>0])
# Task 11:
# Print only the negative values from changes.
print("negative changes:", changes[changes<0])
# Bonus task:
# Print only the values in changes that are not zero.
print("non-zero changes:", changes[changes>0])